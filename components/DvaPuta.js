...
/* ── World Map ───────────────────────────────────────────────── */
// SVG viewBox is 3840×2160 equirectangular
// toX(lon) = (lon+180)/360*3840  toY(lat) = (90-lat)/180*2160
// Crop to Atlantic: lon -100→+50, lat 10→72  →  viewBox="853 216 1600 744"
// NY  74°W 40.7°N  → (1132, 592)
// MI   9.2°E 45.5°N → (2018, 534)
// BG  20.5°E 44.8°N → (2139, 542)

function WorldMap({ vis }) {
  const NY = { x: 1132, y: 592, label: 'New York' };
  const MI = { x: 2018, y: 534, label: 'Milano'   };
  const BG = { x: 2139, y: 542, label: 'Belgrade' };
    ...
    <div className="dp-map-wrap"
      ...
      <svg viewBox="853 216 1600 744" className="dp-map-svg" aria-hidden="true">
        ...
        <g clipPath="url(#dpClip)">
          ...
          <path d={`M${NY.x},${NY.y} Q1640,330 ${BG.x},${BG.y}`}
            fill="none" stroke="rgba(169,135,92,0.1)" strokeWidth="4"
            strokeDasharray="10 14"/>

          {/* Animated arc NY → Milano */}
          <path d={`M${NY.x},${NY.y} Q1575,310 ${MI.x},${MI.y}`}
          ...
          <path d={`M${MI.x},${MI.y} Q2078,470 ${BG.x},${BG.y}`}
